...
#    	,featFile_dev   = "../data/good_vs_bad/CQA-QL-devel.xml.multi.csv.feat"
	...
	print('Loading data...')
	(X_train, y_train), (X_test, y_test), (X_dev, y_dev), (X_newinput, y_newinput), max_features, E, label_id = aidr.load_and_numberize_data(path=options.data_dir, nb_words=options.max_features, init_type=options.init_type, embfile=options.emb_file, dev_train_merge=0, map_labels_to_five_class=0)
...
#	print("Padding sequences....")
	...
	if nb_classes == 2: # binary
		...
		model.fit(X_train, y_train, batch_size=options.minibatch_size, nb_epoch=options.epochs,
				validation_data=(X_dev, y_dev), verbose=2, callbacks=[earlystopper, checkpointer])#-quan bo show_accuracy --> accuracy 
...
######################################################################
	...
	scores = model.evaluate(X_train, y_train)  #quan them vao
	print("%s: %.2f%%" % (model.metrics_names[1], scores[1]*100))#quan them vao

	#get label ids in sorted
	class_labels = sorted(label_id, key=label_id.get)
	#print (class_labels)

	print (metrics.classification_report(y_test, y_pred, target_names=class_labels, digits=5) )
...
###################################################################
	y_newinput_pred = model.predict_classes(X_newinput)
	...
	with open("INPUTlabelPred.csv", "w") as t:
        	writer = csv.writer(t)
