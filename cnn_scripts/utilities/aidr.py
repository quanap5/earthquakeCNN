...
#        print "Reading vocabulary from" + filename
	...
	print (filename)
        for rowid, row in enumerate (reader):
            ...
            elif re.search("newinput.csv", filename.lower()):    
                sentences_newinput.append(row[1])    
                y_newinput.append(row[2])   
...
#        nb_words = len(vocab) 
    ...
    X_newinput,  y_newinput  = adjust_index(X_newinput,  y_newinput,  start_char=start_char, index_from=index_from, maxlen=maxlen)
...
#        y_train=np.concatenate ((y_train, y_dev)) # need if y_train is numpy array
    ...
    return (X_train, y_train), (X_test, y_test), (X_dev, y_dev), (X_newinput,y_newinput), nb_words + index_from, E, label_map
...
def load_data(path="imdb.pkl", nb_words=None, skip_top=0, maxlen=None, test_split=0.2, seed=113,
    ...
    return (X_train, y_train), (X_test, y_test), (X_newinput,y_newinput)
